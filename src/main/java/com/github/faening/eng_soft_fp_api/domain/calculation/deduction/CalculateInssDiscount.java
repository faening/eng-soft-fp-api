...
/*
 * Requisito: [RD014] Calcular Desconto INSS
 *
 * Descrição:
 * Esta classe é responsável por calcular o desconto de INSS de um funcionário em um determinado mês.
 *
 *
 * Funcionamento:
 * Para realizar os cálculos, esta classe observa as propriedades `employee.salary` e `tax_or_value` com o `type`: `INSS`.
 * O desconto de INSS é calculado sobre o salário base do funcionário no mês. O valor é armazenado na tabela `tax_or_value` com o `type`:
 * `INSS`.
 *
 * Exemplos:
 * ...
