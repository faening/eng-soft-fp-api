...
/*
 * Requisito: [RD013] Calcular Desconto Empréstimo (Adiantamento Salário)
 *
 * Descrição:
 * Esta classe é responsável por calcular o desconto por empréstimos ou adiantamentos de um funcionário em um determinado mês.
 *
 *
 * Funcionamento:
 * Para realizar os cálculos, esta classe observa as tabelas `loan` e `installment`.
 * O desconto por empréstimos é calculado com base no valor da parcela do mês atual.
 *
 * Observe que um funcionário pode ter mais de um empréstimo, e o desconto por empréstimos deve o total dos valores das parcelas de todos os
 * empréstimos no mês.
 * Observe que o desconto só pode ocorrer se a propriedade `loan.company_payment_status` for `PAID` (pago).
 *
 * Exemplos:
 * ...
 */
